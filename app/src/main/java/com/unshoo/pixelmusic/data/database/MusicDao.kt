...
private const val EMPTY_SONG_SEARCH_MATCH_QUERY = "pixelmusicemptyquery*"
...
interface MusicDao {
    ...
    // --- Related Song Map Operations (for YouTube autoplay continuation) ---
    @Insert(onConflict = OnConflictStrategy.IGNORE)
    suspend fun insertRelatedSongMaps(maps: List<RelatedSongMap>)

    @Query("SELECT COUNT(*) > 0 FROM related_song_map WHERE song_id = :songId")
    suspend fun hasRelatedSongs(songId: Long): Boolean

    @Query("""
        SELECT artists_json FROM songs
        INNER JOIN favorites ON songs.id = favorites.songId AND favorites.isFavorite = 1
        WHERE songs.source_type = 7 AND artists_json IS NOT NULL
    """)
    ...
    @Query("""
        ...
        LIMIT :limit
    """)
    suspend fun getRelatedSongs(songId: Long, limit: Int = 10): List<SongEntity>

    /**
     * Quick Picks: top recently-played songs weighted by play_count,
